...
**最强大的功能**：将 AI Agent 任务分发到运行 OpenClaw 或 DeerFlow 框架的远程节点。与传统 GPU 插件需要直接访问硬件不同，Agent 插件支持在 AI Agent P2P 网络中分发任务。

### 🤖 Agent 插件

通过 OpenClaw/DeerFlow 集成实现分布式 AI 任务执行：
...
| 框架 | 协议 | 能力 | 适用场景 |
|------|------|------|----------|
| **OpenClaw** | WebSocket (端口 18789) | 任务队列、多渠道 AI 助手、子 Agent 编排 | 自动化、聊天、长时间运行任务 |
| **DeerFlow** | HTTP API (端口 8000) | 深度研究、LangGraph 编排、代码执行、多步骤工作流 | 研究、复杂编码、多 Agent 协作 |
...
**架构设计：**
- **Agent Registry（注册表）**：按能力追踪可用 Agent 节点（web_search、code_execution、deep_research）
- **Task Dispatcher（分发器）**：带并发执行和超时控制的工作池
- **Protocol Adapters（协议适配器）**：支持不同 Agent 框架的可插拔适配器

**定价**：基于任务复杂度、执行时间和所需能力
...
- **Agent 任务分发**: 原生支持 OpenClaw/DeerFlow 框架 - 在 P2P 网络中分发 AI 任务
- **P2P 网络**: 基于 libp2p 的分布式点对点架构
- **插件系统**: 可扩展的插件架构，支持 Agent、GPU、存储和代理资源

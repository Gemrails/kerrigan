...
func (b *RepeaterNodeImpl) GetList(nodeId ...string) (mb []*model.BasicNode) {
	mb = make([]*model.BasicNode, 0)
	rList, err := b.cache.CacheTable(new(model.Repeaters).ModelName()).Value(SELFNODE)
	if err != nil {
		b.log.Errorln("repeater get item error: ", err.Error())
		return
	...
	for e := rList.Data().(*model.Repeaters).NodeList.Front(); e != nil; e = e.Next() {
		...
		inArray := false
		for i, key := range nodeId {
			if key == bp.Data().(*model.BasicNode).NodeInfo.NodeID {
				nodeId = append(nodeId[:i], nodeId[i+1:]...)
				inArray = true
			}
		}
		if len(nodeId) == 0 || inArray {
			mb = append(mb, bp.Data().(*model.BasicNode))
		}
	}
	return
}
